...
| # | 对象 | 真相树归属 | 真源路径 | 验证命令（明细见 §2.2） | 期望值 | 口径版本 | 负责人 | 上次验证 |
|---|------|------------|----------|--------------------------|--------|----------|--------|----------|
| 1 | scheduler.py（JOBS 计数） | 天枢运行区 | `/vol2/1000/software/macro-scan/核心代码/scheduler.py`（容器 `/app/scheduler.py`） | ast 解析（§2.2-1） | 元组 50 = 唯一名 47 + weak_signal×4 | 口径 v1（案例6 方法论，2026-08-06） | devops-governance | 2026-08-06 14:42 实测 50 / 47 / weak_signal dup ✅ |
| 2 | deploy.sh（rsync 标志） | 仓库（天璇 COPY 真源 = repo） | `/vol2/1000/software/world-sim/deploy.sh` | grep（§2.2-2） | `--delete` 计数 0；`rsync -av` 计数 2 | 口径 v1（案例7 修复 18d3962） | devops-governance | 2026-08-06 14:42 实测 0 / 2 ✅ |
| 3 | grv_latest.json（键数/维度/新鲜度） | 天枢数据 | `/vol2/1000/software/macro-scan/data/grv_latest.json`（容器 `/workspace/data`） | jq（§2.2-3） | 顶层键 24；维度分数键 17；`updated` = 当日 | schema v1.0（`_schema_version`=1.0） | arch-governance | 2026-08-06 14:42 实测 24 / 17 / updated 2026-08-06T06:10:18 ✅ |
| 4 | forecast_tracker.db（表计数/行数） | 数据文件（天璇/天玑共享同 inode） | `/vol2/1000/software/macro-scan/data/forecast_tracker.db`（容器 `/app/macro_data`） | docker exec sqlite（§2.2-4） | 业务表 8（+sqlite_sequence=9）；predictions=1、forecasts=289、narrative_chunks=181；actuals/evaluations/weight_update_log=0 **属预期**（唯一预测已 auto-verified，非空转） | schema v1 | arch-governance | 2026-08-06 14:42 实测 9 表 / rows 见 §2.2-4 ✅ |
| 5 | control_server.py（last_ok 逻辑） | 天枢运行区 | `/vol2/1000/software/macro-scan/核心代码/control_server.py` | grep（§2.2-5） | L183 真实健康探测存在；硬编码 `last_ok = True` 计数 0 | 口径 v1（案例4 修复） | devops-governance | 2026-08-06 14:42 实测 L183 / 0 ✅ |
