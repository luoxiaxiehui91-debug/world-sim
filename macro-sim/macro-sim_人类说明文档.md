# 宏观演化仿真系统（macro-sim · 天璇）· 使用与维护手册

> 版本：v2.0.24 | 更新日期：2026-08-07 | 别称：**天璇**（仿真层）

---

## 一、这是什么

macro-sim 是世界推演系统的**仿真引擎层**。它以 macro-scan（数据观测层）每日写入的宏观指标为输入，将17个角色各异的宏观 Agent（12 金融 + 5 主权 S1-S5）放进一个时间轴中，让它们按照各自的信息延迟和传导系数相互影响、演化，最终输出**未来24个月的概率路径树**，以及每条路径的 LLM 叙事说明。
...
同时通过 ntfy 推送摘要到手机（`https://ntfy.sh/macro-tsx-9005`）。
...
| 12个金融 Agent 实现 + 5个主权 Agent（S1-S5） | `core/agents/financial/geopolitical/social.py` + `core/agents/sovereign.py` | ✅ 完成（主权 08-07 激活）|
...
**2. 路径多样性（08-07 更新：A 类激活后已改善）**

08-06 及之前：GRV=80 高压下约 96-99% 路径走向压力区，路径B 概率仅 5%，且无任何主权 Agent 行动——根因已定位为 **12 金融 Agent 全顺周期、缺交易对手段**（非引擎回归）。

08-07 A 类激活（5 个主权 Agent S1-S5 + grv_dimensions + Board + red_line_triggers）后：真实场景（GRV=60.3）已形成路径分叉（A 95% 情绪 -0.77 / B 5% 情绪 -0.22），S 类行动形成结构性多空（S1 美国军事部署 80% run vs S2 中国外交接触 60% run）。**GRV=80 高压仍单路径**（深压一致避险，语义可辩护但 §4 验证标准未达，选项待用户拍板：接受语义 / 继续调参 / 实施 B2 聚类判据增强）。详见 operations 20260807-world-deduction-a-class-activation-implement.md。
