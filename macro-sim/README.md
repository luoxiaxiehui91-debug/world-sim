...
macro-scan 发现信号之后，这里负责**演化未来**：把当前世界状态和 17 个宏观角色（12 金融 + 5 主权 S1-S5）放进去，让它们按各自逻辑互动，结果从演化中涌现出来。
...
| A2 | 商业银行 | 2个月 | 收紧/放松信贷 |
| A3 | 对冲基金 | 即时 | 做空/做多/降险 |
| A4 | 能源国(OPEC+) | 5个月 | **挂起**（activation=0，能源维度已由 S5_saudi 接管） |
...
| S1 | 美国（主权）| 0（试点）| 制裁/军事部署/外交（soul 驱动派系决策）|
| S2 | 中国（主权）| 0（试点）| 台海应对/外交/制裁反制（soul 驱动）|
| S3 | 欧盟（主权）| 0（试点）| 制裁/能源政策/北约（soul 驱动）|
| S4 | 俄罗斯（主权）| 0（试点）| 核威慑/能源武器化/停火信号（soul 驱动）|
| S5 | 沙特-OPEC（主权）| 0（试点）| 产量决策/减产增产（EnergyGov soul 驱动）|
...
| `config/agents.yaml` | 17个 Agent 配置（A1-A12 + S1-S5，热更新，新增角色只加配置）|
