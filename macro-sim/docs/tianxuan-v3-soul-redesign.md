...
### 5.5 参数校准策略（文献区间约束 + 历史数据拟合）

> 用户拍板（2026-08-07 15:38）：**能用以前的数据训练拟合派系权重**——与文献锚点结合为「文献给区间 + 历史数据拟合具体值」标准参数估计流程（先验 + 数据）。

**背景**：29 篇文献锚定触发阈值方向，但派系权重（dove 0.25/hawk 0.20 等）无文献直接支撑（§5.4 标注 `【待校准】`）。系统已有校准循环（run_calibration：50 个月历史拟合 Agent 参数），扩展其可调参数空间即可拟合派系权重。

**拟合机制（三路，A 主 + C 验收 + B 可选）**：

| 路 | 机制 | 说明 |
|----|------|------|
| **A 校准循环扩展（主）** | `apply_param_adjustment` 支持 `internal_factions.{faction}.weight` 参数路径；LLM 调参指令扩展为 `{"agent":"A1","param":"internal_factions.doves.weight","new":0.4}`；prompt 附带文献区间约束（如"dove 权重限 [0.3,0.6]"），LLM 只能在区间内调 | 复用现有 50 个月校准，零新数据源；阶段 2/3 实施 |
| **B 参数扫描（可选强化）** | 对派系权重网格搜索选历史评分最高组合（复用 bleed_params_override sweep 先例）；组合爆炸需降维（一次只扫一个 soul，其余冻结） | 计算量大，作为 A 的补充验证 |
| **C 历史事件回放（验收必做）** | 用已知历史关键事件检验派系行为方向：2022 加息周期→Fed hawk 主导产出 HIKE；2023 硅谷银行→商行 risk_off 收紧；2024 日元套息平仓→BOJ extreme 活跃。方向对=权重可信；方向错=调权重 | 定性验收，是「校验闭环」的天然用例（§7） |

**本质限制（必须知悉）**：
- 历史数据只有**外生变量**（GRV/利差/利率，天枢真实采集）；**内生变量无真实记录**（market_sentiment 等是仿真内部量）→ 拟合的是"行为方向正确的权重"（D2/D3 fix soft target 逻辑），不是真实世界精确权重
- 50 个月历史可能未覆盖极端场景（深压/冲突边缘）→ 训练分布外行为由**文献区间 + red_line_triggers** 兜底（不依赖权重）
...
8. **派系权重历史拟合（§5.5）**：校准循环支持 `internal_factions.*.weight` 调参（A 路）；历史事件回放方向校验通过（C 路：2022 加息/Fed、2023 SVB/商行、2024 套息/BOJ 三个场景方向正确）；拟合后权重回填 §5.3（`【待校准】` → 校准值 + 日期 + 数据窗口），评审可溯源。
...
| 2026-08-07 | **阶段 2 已实施**（commit 6b39d0230，v2.0.26）：试点 3 soul（A1_fed/A3_hedge_fund/A6_media，设计 §4.1/4.3/4.5）+ calibrator 派系权重调参扩展（§5.5 A 路）+ flag_* 派生 + red_line dict 格式 + _eval_trigger abs() 内置函数修复。验收：smoke_v3_phase2.py 6 项全过（历史回放方向校验：2022 Fed hawk 方向正确+权重敏感性、2023 SVB A3 risk_off 主导）+ 阶段 1 回归全过 + 容器内真实数据全过。**待办**：完整校准评分回归对比（试点 ≥ v2 × 0.95，需 2h 校准）；权重回填 §5.3 |
