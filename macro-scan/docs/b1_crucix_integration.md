...
- crucix 最终退场，过渡期仅作为天枢 fetcher 源共存；天枢（macro-scan）是 **唯一数据中枢**（采集 → 落盘契约 → 开阳只读）。
...
> 图例：✅ = 天枢已有等价 fetcher（crucix 可退场）｜⚠️ = 部分覆盖（间接/无专用 fetcher，需决策）｜❌ = 缺口（天枢无对应）

### 2.1 新闻 / 信号聚合
| crucix 维度 | 天枢对应 | 状态 | 备注 / 退场动作 |
|------------|---------|------|---------------|
| 新闻 Feed 聚合 | `fetch_news` (#12) + `fetch_rss_news` (#23 RSSHub) + `fetch_defense_rss` (#21) | ✅ | 三源覆盖通用新闻 + 路由聚合 + 防务垂直；crucix 新闻 Feed 可退场 |
...
| ACLED（冲突事件） | 无 | — | **crucix 自身已放弃**（需 edu/org/gov 授权）；天枢亦无，但 **非退场阻塞项、非缺口** |
...
- 该结论并入本清单，作为 crucix 退场后航空维度"已等价但受限额锁定"的依据。
...
### 4.1 分档统计
- ✅ **已有等价 fetcher（crucix 可退场）**：新闻、利率、汇率、加密、商品、制裁、GPR、能源/EIA、地震、灾害、FIRMS(锚点)、HDX、FAO、气候/ONI、GPR、防务 RSS、SIPRI、BDI、OpenSky(限频)、air
...
1. **FIRMS（已完成）**：`fetch_firms` 纯重写直连已落地，crucix 火点维度退为过渡兜底，可率先独立。
2. **新闻 / 金融 / 灾害 / 气候**：天枢已全覆盖且有等价 fetcher，**crucix 退场无阻塞**。
3. **航空（OpenSky）**：天枢已等价，但受匿名 400 credits/日限额锁定为日档（绝不可提频）——退场无影响，因天枢已独立且频率本就日档。
4. **部分覆盖项（gscpi / nuke / GDELT / sdr）**：D2 需拍板是否新增专用 fetcher，或接受间接代理。若接受间接代理，则 crucix 可整体退场；若要求专用信号，则先行补齐再退场。
5. **ACLED**：非阻塞，忽略。

> **总体判断**：crucix 绝大多数"信息收集能力"天枢已通过 24 个 fetcher 等价或覆盖；仅 `gscpi`/`nuke`/GDELT 为部分覆盖，不构成退场硬阻塞，D2 可按"先退场、部分维度接受间接代理"推进，或视世界推演精度需求择机补齐。
