...
- **NAS crontab**：新增 `0 10 * * * NTFY_TOPIC=macro-tsx-9005 python3 .../check_doc_drift.py`
...
- `核心代码/update_kb_numbers.py` 第26行：`NTFY_TOPIC` 默认值从硬编码的 `"macro-tsx-9005"` 改为 `""`，与 `daily_narrative.py` / `situation_detector.py` / `weekly_synthesis.py` 行为一致（无环境变量时静默跳过推送）
...
**实现细节：**
- 监听：`GET https://ntfy.sh/macro-tsx-9005-cmd/sse`（SSE 长连接，断线自动重连）
- 密钥校验：首词不匹配则静默丢弃
...
- **问题**：`FRED_API_KEY = "a3f1dc8fca52b0a45e320ea0383bbac7"` 硬编码在方法内部，无法通过环境变量覆盖。
...
**环境变量：** NTFY_TOPIC=macro-tsx-9005 ✅，OPENCLAW_WORKSPACE=/workspace ✅
...
**手机 ntfy（向 macro-tsx-9005-cmd 发消息）：**
```
