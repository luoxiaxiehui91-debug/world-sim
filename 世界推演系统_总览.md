...
> macro-scan v3.5.49 · macro-sim v2.0.3 · 2026-07-11
...
FRED / GPR / GDELT / 新闻（RSSHub + Crucix）
        ...
        ┌─────────────┐
        │  macro-scan  │  数据观测层
        │  v3.5.47    │  每日定时采集 → GRV向量 → LLM分析报告 → ntfy手机
        └──────┬──────┘
               │ GRV告警时写 sim_trigger.json
               ▼
        ┌─────────────┐
        │  macro-sim  │  仿真引擎层（早期开发阶段）
        │  v2.0.2     │  Monte Carlo×100 → 概率路径树 → ntfy手机
        └─────────────┘
```

| 子系统 | 定位 | 容器模式 | NAS路径 | 详细文档 |
|--------|------|----------|---------|---------|
| macro-scan | 数据观测层 | 热挂载（改代码即生效） | `/vol2/1000/software/macro-scan` | `macro-scan/世界推演系统_人类说明文档.md` |
| macro-sim | 仿真引擎层 | COPY模式（改代码需重建镜像） | `/vol2/1000/software/macro-sim` | `macro-sim/macro-sim_人类说明文档.md` |
