...
> 版本：Wave 2 · v1.11.25 对齐版 ｜ 更新日期：2026-08-16
> 配套数据权威标准：同目录 [`DATA_CONTRACT.md`](./DATA_CONTRACT.md)
> 接手快照：同目录 [`NEXT_SESSION_HANDOFF.md`](./NEXT_SESSION_HANDOFF.md)（1.11.25，含 08-16 大迭代版本链）
...
| P1：新闻地理化上图 / 冲突事件图层 | 新闻地理化 ✅ **已上线（1.9.0，news_geo GDELT geo feed）**；冲突事件 ⏸ 等 ACLED（已放弃 → UCDP 评估） |
...
| **事件弹框 + 同地点聚合** | ✅ **已交付（v1.10.5 → 1.10.8）：点击新闻/冲突/卫生点弹框（EventPopup）+ 聚合组 children（bbox + 拼写变体合并）** |
| **卫生图层**（GDELT 卫生事件 + 中文疾病名 + 新闻关联） | ✅ **已交付（1.9.0 图层 → 1.11.16-18 功能链）：source_media 媒体名 + DISEASE_ZH 中文映射 + 点击弹框原文链接** |
| **新闻标题真实化**（预抓 + LLM 翻译） | ✅ **已交付（1.11.19-23）：news_titles.json 预抓缓存（I120）+ MiMo 中文翻译 + 控制 API /news-title 兜底；三级取数弹框秒开** |
| **地图交互**（背景取消选中） | ✅ **已交付（1.11.24-25）：点击空白/关弹框取消选中；点位 click 阻断冒泡（回归修复）** |
| **控制台 Token 体系** | ✅ **已交付（1.11.12-15）：Token 配置 UI + 401 内联重试 + 构建注入 VITE_CONTROL_API_TOKEN + H02 信任校验** |
| P2：空域 / 热异常 / 海上 / SDR 图层、风险仪表、sweep delta | ⏸ 待后端新建 feed（④空域⑤热异常天枢已有基础可优先） |
