...
## [1.11.34] - 2026-08-18 · 天玑 Tab：预测判据显示（配合天璇描述清晰化）

**修改理由**：用户指出预测描述模糊（"中性报道是什么""对谁做空"）——天璇侧已为
geo 预测的 outcome_definition 补充现实判定标准（`10fdf202`），开阳侧显示判据行。

### 修改

- **`control/TianjiTab.tsx`**：待人工验证列表新增"判据"行——从 `outcome_definition`
  中提取"判定标准："之后的内容显示（10px 灰字，完整换行）；旧预测无判据不显示

### 验证

- `tsc --noEmit` 通过；vite build（`index-Dg-NZgNS.js`）；线上 :8080 生效
- C1 试验新预测实测：content `路径A路径（GRV 60→62）：媒体放大恐慌情绪（仿真频率44%）`，
  outcome `…判定标准：主流财经媒体负面报道占比连续≥2个月≥40%（参考：VIX 读数/新闻情绪指数）`
...
看不出占比分布和触发率。做成结构化卡片——占比条 + run 级触发率 badge + 接任标签，
一眼看清每个主权更迭行为。

### 修改

- **`lib/governance.ts`（新增）**：`extractGovernanceEvents(md)` 纯函数——从报告 md 提取
  "## 政权更迭事件"节，解析每主权 类型占比（换届转向/延续/继承政变）/ runs 触发率 /
  发生月 / 接任 regime 标签；返回 `{items, rest}`（rest = 剥离节后的正文）；解析失败返回
  null（调用方降级纯文本，解析器是增强不是依赖）
- **`components/GovernanceCard.tsx`（新增）**：卡片——头部（标题 + "Monte Carlo 100 runs 聚合"
  徽标）+ 每主权行（国家 + runs 触发率 badge[≥50% amber/其余 teal] + 三段占比条
  [转向 teal / 延续 灰 / 继承政变 红] + 发生月 + 接任标签）+ 底部图例
- **`components/ReportsPanel.tsx`**：md 渲染前解析 gov（useMemo 缓存一次），成功 → 卡片 +
  剥离节后正文；失败 → 原样全文渲染
- **`control/TianxuanTab.tsx`**：报告全文渲染同机制接入（天璇 Tab 与报告面板体验一致）

### 验证

- node 实测解析：美国/日本/韩国/俄罗斯/沙特 5 行全部正确（类型占比/runs%/月/标签），
  节剥离后 rest 保留后续正文；无节 md 返回 null（降级 OK）
- `tsc --noEmit` 通过；vite build（`index-GMcEIOOa.js`）
- 线上 :8080 实测：新 bundle 生效，含"政权更迭事件 / 实质更迭"文案
- 纯前端 + 纯文本解析：历史报告（G1/G2/G3/J1）同样生效，零后端改动
...
- **`components/ReportsPanel.tsx`**：
  - **全开/全关**：标题栏新增 SVG 按钮（双下箭头=展开全部 / 双右箭头=收起全部），与既有单组折叠共存——无折叠时点击收起全部组，有折叠时点击展开全部
  - **近期过滤**：标题栏新增时间分段 `[全部 | 近7天 | 近30天]`，按 `updated`（YYYY-MM-DD）裁剪列表；分组、计数、选中项自动跟随（原选中被裁掉时重置到过滤后第一条）
  - **"今天"高亮**：列表项日期行——今天的报告以强调色 teal 显示"今天"，其余显示日期灰字，一眼定位最新
  - 日期解析显式拆分量（`parseDateOnly`），规避 JS 对 date-only 字符串按 UTC 解析的坑

### 验证

- `tsc --noEmit` 通过；vite build（`index-DGIcKLqU.js`）
- 线上 :8080 实测：新 bundle 生效，含"近7天 / 展开全部分组 / 收起全部分组"文案
